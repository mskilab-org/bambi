...
bxBam = R6Class('bxBam',
    ...
    public = list(
        ...
        initialize = function(bamdb, bam){   

            if (!file.exists(bamdb)){
                stop('Error: Pathname to bamdb LMDB subdirectory must be provided') ### write in checks that it's actually an LMDB subdirectory 
            }   

            self$bamdb = bamdb
            self$bam = bam    ### write in checks that BAM exists with RSamtools
        },
